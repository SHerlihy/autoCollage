...
describe("generateEdgesMap", ()=>{
  ...
  describe('rectangle with crevice', ()=>{
    ...
    it('returns one crevice', ()=>{
      ...
      expect(edgeCreviceLeft?.type).to.deep.equal({from: 'crevice', to: 'crevice'})
      expect(edgeCreviceRight?.type).to.deep.equal({from: 'crevice', to: 'crevice'})
    })
  ...
  describe('star shape', ()=>{
    ...
    it('returns all right angles as crevice', ()=>{
      ...
      for (const [returnedEdgeKey, returnedEdgeValue] of returnedEdgesMap) {
        const nextReturnedEdgeValue = returnedEdgesMap.get(returnedEdgeValue.nextEdge)
        expect(returnedEdgeValue.type.to).to.equal(nextReturnedEdgeValue?.type.from);
      }
    })
  })
})
