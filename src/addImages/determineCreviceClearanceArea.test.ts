...
import { determineCreviceClearanceArea, IClearanceArea, withinEpsilonBounds } from "./determineCreviceClearanceArea"

describe("assignImgsToEdges", ()=>{
  const expectClearanceCoordinatesFromCreviceCoordinates = (creviceTL: ICoordinates, creviceBottom: ICoordinates, creviceTR: ICoordinates, minWidth: number, expectedClearanceCoordinates: IClearanceArea) =>{
    const returnedClearanceCoordinates = determineCreviceClearanceArea(creviceTL, creviceBottom, creviceTR, minWidth)

    expect(returnedClearanceCoordinates).to.not.equal(undefined)

    if(!returnedClearanceCoordinates) return;

    for (const [expectedKey, expectedCoordinates] of Object.entries(expectedClearanceCoordinates)) {
      const {x: expectedX, y: expectedY} = expectedCoordinates;
      const {x: returnedX, y: returnedY} = returnedClearanceCoordinates[expectedKey];
  ...
  describe("right angle isosceles crevices ", ()=>{
    it('returns expected coordinates for upward facing', ()=>{
      ...
      expectClearanceCoordinatesFromCreviceCoordinates(creviceTL, creviceBottom, creviceTR, minWidth, expectedClearanceCoordinates)
    })
